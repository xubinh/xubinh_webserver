...
| ----------------------------------------------------------------------------------------------- | ---------- | ---------- | ------------------------------------------ |
| 初次测试稳定后                                                                                  | 38,661     | 84,392     | `2794336a6d619f14d15ef84f438e6b60ec934310` |
| 为每个工作线程在主线程中独立配备阻塞队列                                                        | 37,852     | 80,460     | `c48a4075680bf022096cc6e4103ac98512d669dd` |
...
| 统一 TCP 连接的缓冲区的初始大小与读事件处理函数中的扩展大小, 避免首次读取时的无意义的内存重分配 | 42,147     | 99,664     | `904f5dbf257e07c46a845dd34a2f50b184056c7c` |

#### 横向比较

| 项目名称                                                   | 短连接 QPS | 长连接 QPS | commit                                     |
| ---------------------------------------------------------- | ---------- | ---------- | ------------------------------------------ |
| [linyacool/WebServer](https://github.com/xubinh/WebServer) | 37,814     | 75,945     | `a50d635f48178c89f78b4be9d2579613b2c7debf` |
