...
- [x] `FunctorQueue`: 函子专用的阻塞队列, 用于 `EventLoop` 类中作为其成员.
...
- [x] `TimerContainer`: 对定时器容器的抽象. 内部采用树的实现方式.
  - 为了效率起见, 内部不使用智能指针而是直接对裸指针进行操作. 同时使用裸指针 (实际上是 `TimerIdentifier` 类对象) 也便于定位定时器.
  - 提供的 API:
    - 添加一个定时器.
    - 取消一个定时器.
    - 获取在给定时间点前超时的所有定时器.
...
- [x] `FileDescriptor`: 对文件描述符的抽象, 作为各个分化的文件描述符类型的公共基类. 其成员仅包含一个文件描述符 `_fd` 和一个 `EventDispatcher` 类型的对象.
...
- [ ] `EventLoop`: 用于收纳事件循环相关的代码.
  ...
  - 提供的 API 包括:
    ...
    - 向定时器容器中添加或删除定时器, 并利用 timerfd 设置或修改闹钟.
